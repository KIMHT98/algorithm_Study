...
SELECT 
    manufacturer.company_name, COUNT( modelname) AS '개수'
FROM
    car
        RIGHT JOIN
    manufacturer ON car.manufacturer_code = manufacturer.code
...
SELECT 
    manufacturer.nation, COUNT(vin) AS '개수'
FROM
    car
        LEFT JOIN
    manufacturer ON car.manufacturer_code = manufacturer.code
...
# 8.	모델명이 '소나타' 인 차량의 주행거리 이상인 모델명과 주행거리를 출력하시오.
SELECT 
    modelname, mileage
FROM
    car
...
# 10.	주행 거리가 20000 이상인 차량들 중 차량번호에 ‘11’ 을 포함하는 차량의 모델명과 차량번호, 주행거리를 출력하시오.
SELECT 
