...
select COUNT(*) as '행수' from user, board;

#2.	모든 board가 누가 작성했는지 알 수 있도록 작성자의 id와 title을 출력하시오.
select user.id, board.title from user , board WHERE user.name = board.writername;
...
select user.email, board.createtime from user INNER JOIN board ON user.name = board.writername where YEAR(createtime) < '2018';

#5. 각 년도에 몇명의 사용자들이 board를 작성했는지 출력하시오. 한명이 어떤 년도에 2개이상 작성했다면 1번만 센다. ( 중복 제거 )
select COUNT(DISTINCT user.id) as '사용자 수', YEAR(createtime) as '년도' from user INNER JOIN board ON user.name = board.writername GROUP BY YEAR(createtime);

#6. 2016년 이후 각 사용자들이 몇개의 board를 작성했는지 알 수 있도록 id와 작성한 board의 개수를 출력하시오.
select user.id , COUNT(createtime) as '개수' from user INNER JOIN board ON user.name = board.writername where YEAR(createtime) > '2016' GROUP BY user.id;
